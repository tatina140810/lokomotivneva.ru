  ...
  w.renderAdsBox = function (host, opts) {
    ...
    function budgets() {
      ...
      return '<table><thead><tr><th>Статья бюджета</th><th>Потрачено</th><th>Переходов</th>'
        ...
        + '<p class="t-sub" style="margin:10px 0 0">Как считается «потрачено»: '
        + 'между двумя замерами остатка — точно (прошлый остаток плюс пополнения минус новый). '
        + 'Если внесён расход за длинный период, за выбранные дни берётся его доля по дням — '
        + 'это среднее, а не факт: площадка тратит неравномерно. Чем чаще вносите остаток, '
        + 'тем точнее цифра.</p>';
    }
