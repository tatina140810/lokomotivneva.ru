...
(function (w, d) {
  ...
  /* Выключатель для этого браузера. Ставится кнопкой на странице статистики и нужен
     там, где адрес меняется (мобильный интернет, чужой Wi-Fi) и исключение по адресу
     не помогает. Хранится в самом браузере, поэтому включается на каждом устройстве
     отдельно. */
  try {
    if (w.localStorage.getItem('loko_stat_off') === '1') return;
  } catch (e) { /* хранилище закрыто — считаем как обычно */ }
